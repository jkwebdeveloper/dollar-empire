...
import { handleGetPrivacyContent } from "../redux/GetContentSlice";

const PrivayPolicy = () => {
  const { t } = useTranslation();
  const { loading, privacyNotice } = useSelector((state) => state.getContent);

  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(handleGetPrivacyContent());
  }, []);
  return (
    <>
