...
import { handleGetShippingAndFreightContent } from "../redux/GetContentSlice";

const ShippingAndFreight = () => {
  const { t } = useTranslation();

  const { loading, shippingAndFreight } = useSelector(
    (state) => state.getContent
  );
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(handleGetShippingAndFreightContent());
  }, []);
...
export default ShippingAndFreight;
